...
- **Plan 1 — dev-API faucet** (`docs/superpowers/plans/2026-06-23-teratestnet-faucet-dev-api.md`): address-in → broadcast → EF, plus the web UI and abuse prevention, on the wallet-toolbox engine. Ship first. **Shipped.**
- **Plan 2 — BRC-100 wallet onboarding** (`/api/claim/wallet`, BEEF + remittance, client `internalizeAction`): **implemented** — `payToWallet`/`claimToWallet` + the `/api/claim/wallet` route + the `WalletClaim` "Send to my wallet" UI. The server payout and BEEF assembly are unit-tested; the user-side SPV path (a teratestnet-capable consumer wallet accepting our BEEF on `internalizeAction`) is validated by the manual E2E in §9.
